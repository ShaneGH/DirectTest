...
namespace Dynamox.Tests.Compile
{
    ...
    public class CompileTests2
    {
        ...
        public class XXXXX
        {
            ...
            public virtual int DoSomething(int val)
            {
                return val;
            }
        }
        ...
        {
            var subject = (XXXXX)
                Compiler2.Compile(typeof(XXXXX)).GetConstructors()[0].Invoke(new object[] { new ObjectBase(new ReadOnlyDictionary<string, object>(new Dictionary<string, object>()
                )) });

            var op = subject.DoSomething(66);
